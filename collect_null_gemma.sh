#!/usr/bin/env bash
#SBATCH --job-name=collect_null_gpt
...
BATCH_SIZE=10
N_ROWS=181
OUTPUT_FILE="/scratch/kumar.sag/data/interim/null_measurements_${MODEL##*/}_${TEMP}_wp.ndjson"

for START in $(seq 0 $BATCH_SIZE $((N_ROWS-1))); do
  END=$((START + BATCH_SIZE - 1))
  echo "Running batch: $START to $END"
  python -u winogender_contextuality/modeling/collect_sequential.py generate-one-null-context \
    "$DEVICE" "$MODEL" "$TEMP" --n-runs="$TOTAL_RUNS" \
    --start "$START" --end "$END" \
    --input-file="all_sentences_wp.csv" \
    --output-file "$OUTPUT_FILE" --no-quantized
done
